...
1. The responses are not equally spaced apart. Neutral to agree is not the same step or difference as agree to strongly agree. Using the paired t-test, we are assuming the same difference between each step on the likert scale, which is fundamentally incorrect. We are measuring something that is not linear on a linear scale.

2. The larger implication is that we are averaging answers. If one responds that they strongly agree, and one responds neutral, we are assuming that on average people agree when this does not accurately reflect the intent of the respondents, mischaracterizing our customers opinions of our websites.

# Test Assumptions
##1. Assumptions for a two-sampled t-test to evaluate whether countries with high GDP per capita (higher than the mean) are more or less happy than people in countries with low GDP (lower than the mean)
...
2. Assumptions, and evaluation of assumptions, for a Wilcoxon rank-sum test on whether Democratic or Republican senators are older.
...
```{r}
library(ggplot2)
ggplot(data=legis, aes(birthday)) + 
  geom_bar() 
```
...
```{r Read Wine dataset, echo=FALSE}
data('wine')
...
3. Assumptions, and evaluation of assumptions, for a signed-rank test on whether countries have more deaths from heart or liver disease based on wine consumption.
...
## Assumptions for signed test (Wilcoxon Rank Sign Hypothesis of Means Test)
    - Non-parametric
...
4. Assumptions, and evaluation of assumptions, for a paired t-test and evaluate them.
    

```{r GSS_religion, echo=FALSE}
GSS_religion <- read_csv("GSS_religion.csv")
...
```{r Read General Social Survey religion data, echo=FALSE}
